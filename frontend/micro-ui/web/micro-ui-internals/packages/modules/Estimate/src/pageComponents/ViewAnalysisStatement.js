...
const ViewAnalysisStatement = ({ formData, ...props }) => {
    ...
    const isCreateOrUpdate = /(measurement\/create|estimate\/create-detailed-estimate|estimate\/update-detailed-estimate|measurement\/update|estimate\/create-revision-detailed-estimate|estimate\/update-revision-detailed-estimate)/.test(window.location.href);
    ...
    async function callCreateApi() {
        ...
        let payload = {
            "statementRequest": {
                "tenantId": tenantId,
               "id":formData?.SORtable?.[0]?.estimateId
            }
        }

        {
            await AnalysisMutation(payload, {
              ...
              onSuccess: async (responseData, variables) => {
                ...
                setTimeout(() => {history.push(`/${window?.contextPath}/employee/estimate/view-analysis-statement`, responseData)}, 5000);
              },
            });
          }
    }
